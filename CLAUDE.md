...
- **Stack a picture treatment on the rendered output.** `add_processor streamlib_doom.treatments:PictureTreatment` with `{"treatment": "bloom", "amount": 1.0}`, then connect the current end of the chain into its `frame_from_upstream`, disconnect that end from the console, and connect your new node's `frame_to_downstream` to the console's `neural_from_upstream`. Treatments are bloom, scanlines, outline, grade, chromatic, vignette, sharpen, posterize, underwater. They stack, so ask for two and splice two. Remove one and the picture goes back.
