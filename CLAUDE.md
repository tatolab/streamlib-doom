...
- Spawn: `{"command":"spawn","kind":"imp","count":3,"where":"behind","distance":400}` — kind imp|zombieman, where behind|ahead|left|right, distance 64..900 map units (128 is in your face, 400 is across the room and in shot).
- Clear: `{"command":"clear"}` — every monster off the level at once.
...
- Face: `{"command":"face"}` turns the marine toward the nearest monster, for when you want what you spawned in shot.
...
- **Read the transcript instead of the screen.** `curl "http://127.0.0.1:8670/transcript?since=N"` returns only what happened since line N, in sentences. Use it rather than pulling snapshots; a frame costs far more than a line and the graph has already watched every one.
- **Stack a picture treatment on the rendered output.** `add_processor streamlib_doom.treatments:PictureTreatment` with `{"treatment": "bloom", "amount": 1.0}`, then, in this order because the console holds at most eight inbound links: **disconnect** the current end of the chain from the console's `neural_from_upstream` first, connect that end into your new node's `frame_from_upstream`, and only then connect your new node's `frame_to_downstream` to the console's `neural_from_upstream`. Connecting before disconnecting fails and leaves a stuck link. Treatments are bloom, scanlines, outline, grade, chromatic, vignette, sharpen, posterize, underwater. They stack, so ask for two and splice two. Remove one and the picture goes back.
...
There is no verb for everything. If what you were asked does not map onto one of the commands above, say so in your reply and do the closest thing you can — a command the game does not know is answered with `cannot do '<verb>'` on the HUD, and leaves a processor on the graph that looks like it worked.
